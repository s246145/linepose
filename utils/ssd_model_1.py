...
class SSD(nn.Module):
    ...
    def forward(self, x,targets):
       ...
       #         print(x.size())
        ...
        ignore_mask = ignore_t < 1 # torch.Size([num_batch, 8732])
        pos_idx = pos_mask.unsqueeze(pos_mask.dim()).expand_as(ignore_mask).to(self.device,non_blocking=True)
        
        ignore_mask = ignore_mask[pos_idx].view(-1,1).to(self.device,non_blocking=True)

        ignore_idx = ignore_mask.unsqueeze(ignore_mask.dim()).expand_as(ignore_mask).to(self.device,non_blocking=True)

        pose_p = pose_sources[ignore_idx].view(-1, 256).to(self.device,non_blocking=True)
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        loc_data,loc_t,conf_data,conf_t_label,ignore_t,line_p,pose_p,pose_t,dbox_list = predictions
