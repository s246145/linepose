...
class Detect():
    ...
    def __call__(self, loc_data, conf_data,pose_list, line_list,dbox_list):
        ...
        return output  # torch.Size([1, 2, 200, 5+32+4])
...
class SSD(nn.Module):
    ...
    def forward(self, x,targets):
        ...
        for (x, l,c,li) in zip(sources, self.loc, self.conf,self.line):
            ...
            line.append(F.relu(li(x)).permute(0, 2, 3, 1).contiguous())
        ...
        line = line.view(line.size(0), -1, 256)
        ...
        pose_p = line[pos_idx].view(-1, 256).to(self.device,non_blocking=True)
        ...
        if self.phase == "inference":  # 推論時
            ...
            return line_p,conf_line
            # 返り値は(loc, conf, dbox_list)のタプル

class SSD_pre(nn.Module):

    def __init__(self, phase, cfg,device='cuda:0'):
        super(SSD_pre, self).__init__()
    ...
    def forward(self, x):
        ...
        line = line.view(line.size(0), -1, 256)
        print(line[0].size())
        for idx in range(line.size(0)):

            line_p = self.line_1[0](line[idx]).to(self.device,non_blocking=True)
            pose_p = F.relu(self.line_1[0](line[idx])).to(self.device,non_blocking=True)

            pose_p = F.relu(self.line_1[1](pose_p)).to(self.device,non_blocking=True)
            pose_p = self.line_1[2](pose_p).to(self.device,non_blocking=True)

            line_p = line[idx].unsqueeze(dim=0)
