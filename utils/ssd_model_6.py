...
def make_loc_conf(num_classes=2, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):
    ...
    pose_layers += [nn.Conv2d(512,bbox_aspect_num[0]*32, kernel_size=3, padding=1)]
    ...
    conf_layers += [nn.Conv2d(1024, bbox_aspect_num[1]
                              * num_classes, kernel_size=3, padding=1)]
    pose_layers += [nn.Conv2d(1024,bbox_aspect_num[1]*32
    ...
    conf_layers += [nn.Conv2d(512, bbox_aspect_num[2]
                              * num_classes, kernel_size=3, padding=1)]
    pose_layers += [nn.Conv2d(512,bbox_aspect_num[2]*32
                             , kernel_size=3, padding=1)]
    ...
    conf_layers += [nn.Conv2d(256, bbox_aspect_num[3]
                             * num_classes, kernel_size=3, padding=1)]
    pose_layers += [nn.Conv2d(256,bbox_aspect_num[3] * 32
                            , kernel_size=3, padding=1)]
    ...
    conf_layers += [nn.Conv2d(256, bbox_aspect_num[4]
                              * num_classes, kernel_size=3, padding=1)]
    pose_layers += [nn.Conv2d(256,bbox_aspect_num[4] * 32,
                            kernel_size=3, padding=1)]
    ...
    conf_layers += [nn.Conv2d(256, bbox_aspect_num[5]
                              * num_classes, kernel_size=3, padding=1)]
    pose_layers += [nn.Conv2d(256,bbox_aspect_num[5] * 32
                              , kernel_size=3, padding=1)]
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg,device='cuda:0'):
        ...
        self.L2Norm = L2Norm()
        self.loc, self.conf, self.line, self.line_1,self.line_2= make_loc_conf(
            2, cfg["bbox_aspect_num"])
    ...
    def forward(self, x,targets):
        ...
        for (x, l,c,li) in zip(sources, self.loc, self.conf,self.line):
            # Permuteは要素の順番を入れ替え
            loc.append(l(x).permute(0, 2, 3, 1).contiguous())
            conf.append(c(x).permute(0, 2, 3, 1).contiguous())
            line.append(li(x).permute(0, 2, 3, 1).contiguous())
            linep.append(F.relu(li(x)).permute(0, 2, 3, 1).contiguous())
        ...
        linep = torch.cat([o.view(o.size(0), -1) for o in linep], 1)
        ...
        linep = line.view(linep.size(0), -1, 32)
        num_batch = loc.size(0)  # ミニバッチのサイズ
        num_dbox = loc.size(1)  # DBoxの数 = 8732
        num_classes = conf.size(2)  # クラス数 = 16
        #print(loc.size())
        #print(conf.size())
        ...
        for idx in range(num_batch):  # ミニバッチでループ
            ...
            # ラベル [物体1のラベル, 物体2のラベル, …] 0始まりにするために-1
            labels = (targets[idx][:, 4] - 1).to(self.device,non_blocking=True)
            # ポーズ[物体1のポーズ、物体2のポーズ, ...]
        ...
        pos_idx = pos_mask.unsqueeze(pos_mask.dim()).expand_as(line).to(self.device,non_blocking=True)
        
        line_p = line[pos_idx].view(-1, 32).to(self.device,non_blocking=True)
        linep_p = linep[pos_idx].view(-1, 32).to(self.device,non_blocking=True)
        #line_p = self.line_1[0](pose_p).to(self.device,non_blocking=True)
              
        pose_p = F.relu(self.line_1[1](linep_p)).to(self.device,non_blocking=True)
        ...
        pose_z[0:,3] = torch.cos(pose_p[0:,3]/2)
        #print(conf.size()) 
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        conf_l_label_hnm = conf_l_label[(pos_mask+neg_mask).gt(0)]

        # confidenceの損失関数を計算（要素の合計=sumを求める）
        loss_c = F.cross_entropy(conf_hnm, conf_l_label_hnm, reduction='sum')
