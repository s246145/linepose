...
class Detect():
    ...
    def __call__(self, loc_data, conf_data,line_list, pose_list,dbox_list):
        """
        順伝搬の計算を実行する。
        ...
        for i in range(num_batch):
            ...
            for cl in range(1, num_classes):
                ...
                output[i, cl, :count] = torch.cat((scores[ids[:count]].unsqueeze(1),
                                                   boxes[ids[:count]],lines[ids[:count]],poses[ids[:count]]),1)
...
class SSD(nn.Module):
    ...
    def forward(self, x):
       ...
       #         print(x.size())
        ...
        #print(pose_sources.size())
        #print(len(pose_sources))
        for i in range(num_batch):
            line.append(self.line_1[0](pose_sources[i].unsqueeze(dim=0)))
            #print(self.line_1[0](pose_sources[i]).unsqueeze(dim=0))

            pose_p = F.relu(self.line_1[0](pose_sources[i]))
            pose_p = F.relu(self.line_1[1](pose_p))
            pose.append(self.line_1[2](pose_p).unsqueeze(dim=0))
        # さらにlocとconfの形を整える
        # locのサイズは、torch.Size([batch_num, 8732, 32])
        # confのサイズは、torch.Size([batch_num, 8732, 4])
        #line = torch.cat([o.view(num_batch, -1) for o in line], 1)
        #pose = torch.cat([o.view(num_batch, -1) for o in pose], 1)
        line = torch.cat(line, dim=0)
        pose = torch.cat(pose, dim=0)
        #print(line.size())
        #print(pose.size())
        #line = line.view(line.size(0), -1, 32)
        #pose = pose.view(pose.size(0), -1, 4)
        
        #pose[0:,0] = pose_y[0:,0] * torch.sin(pose_p[0:,3])
        #pose[0:,1] = pose_y[0:,1] * torch.sin(pose_p[0:,3])
        #pose[0:,2] = pose_y[0:,2] * torch.sin(pose_p[0:,3])
        #pose[0:,3] = torch.cos(pose_p[0:,3])
        
        output = (loc, conf,line,pose,self.dbox_list)
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        #ig_idx = ig_mask_1.unsqueeze(ig_mask_1.dim()).expand_as(loc_p)

        #ignorekarakyousidata wosyutoku
        #loc_p = loc_p[ig_idx].view(-1,4)
        #loc_t = loc_t[ig_idx].view(-1,4)
        ...
        #ig_idx = ig_mask_1.unsqueeze(ig_mask_1.dim()).expand_as(conf_p)
        #conf_p = conf_p[ig_idx].view(-1,1)
        # ----------
        # クラス予測の損失：loss_cを計算
        ...
        #pos_idx = pos_mask.unsqueeze(pos_mask.dim()).expand_as(line_data)
        ...
        #pos_idx = pos_mask.unsqueeze(pos_mask.dim()).expand_as(conf_t_label)
        
        #conf_l = conf_t_label[pos_mask].view(-1,1)

        #ig_idx = ig_mask_1.unsqueeze(ig_mask_1.dim()).expand_as(conf_l)
        #conf_pt = conf_pt_label[pos_mask_1].view(-1,1)
        #conf_l = conf_l[ig_idx].view(-1,1)
        #print(conf_l.size())
        #conf_l = conf_l.squeeze()
        #pos_idx = pos_mask.unsqueeze(pos_mask.dim()).expand_as(line_data)
        ...
        #a13,p3,a23,n3 = miner_3(line_p.to(torch.float32),conf_l)
        #A = miner_3(line_p.to(torch.float32),conf_l)
        ...
        #loss_t = loss_func(line_p.to(torch.float32),conf_l,A)
