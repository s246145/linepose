...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.phase == "inference":  # 推論時
            ...
            return output
            # 返り値は(loc, conf,line,pose,dbox_list)のタプル
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        miner_1 = miners.PairMarginMiner(distance = distance,pos_margin=0, neg_margin=0)#ペアマイナ―
        miner_2 = miners.TripletMarginMiner(distance = distance,margin=5.0,type_of_triplets="semihard")#トリプレットマイナー

        loss_func = losses.TripletMarginLoss(distance = distance,margin=5.0)#トリプレットロス
        ...
        #loss_func = losses.TripletMarginLoss(margin=0.2)#トリプレットロス
        #loss_func = losses.TripletMarginLoss(margin=1.0)#トリプレットロス
